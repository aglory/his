...
create table `Role` (
  ...
  `IsInner` int not null comment '是否为内部角色',
  `CreateDate` DateTime NULL,
  primary key (`Id`)
) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8 COLLATE = utf8_bin comment = '角色信息';
drop table if exists `RoleUser`;
create table `RoleUser` (
  `UserId` int not null,
  `RoleId` int not null,
  primary key (`UserId`, `RoleId`)
) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8 COLLATE = utf8_bin comment = '用户角色关联关系';
...
create table `Content` (
  ...
  `CreateDate` DateTime NULL,
  primary key (`Id`)
) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8 COLLATE = utf8_bin comment = '内容';