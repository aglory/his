...
$sql = 'select SQL_CALC_FOUND_ROWS ' . implode(',', $columns) . ' from ViewMemberIntegralHistory';
if (count($sqlWhere)) {
...
try {
  ...
  $sql = 'select count(1) as total,sum(Amount * TypeSign) Amount from ViewMemberIntegralHistory';
  if (count($sqlWhere)) {
    $sql = $sql . ' where ' . implode(' and ', $sqlWhere);
  }
  $sql = $sql . ';';
