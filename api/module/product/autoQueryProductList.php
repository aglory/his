...
foreach ($_POST as $key => $value) {
  if (in_array($key, $columnEqual)) {
    ...
    $sqlWhere[] = '(Code like :Code or ShortName like :ShortName)';
    $sqlParams['Code'] = "%$value%";
    $sqlParams['ShortName'] = "%$value%";
  }
}
...
$sql = 'select ' . implode(',', $columns) . ' from Product';
if (count($sqlWhere)) {
  $sql = $sql . ' where ' . implode(' and ', $sqlWhere);
}

// 分页统一参数
$sql = $sql . ' order by length(ShortName) asc limit 0, 20;';
