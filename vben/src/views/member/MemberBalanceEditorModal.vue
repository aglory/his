<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="充值" @ok="handleSubmit">
...
<script lang="ts">
  ...
  import { changeBalanceFormSchema } from './MemberManager.config';
  import { MemberManagerResponse } from '/@/api/member/model/memberModel';
  import { memberChangeBalanceApi } from '/@/api/member/member';

  export default defineComponent({
    name: 'MemberBalanceEditorModal',
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          ...
          await memberChangeBalanceApi(values);
          closeModal();
          emit('success');
        } catch (ex: any) {
          console.info(ex);
